...
## After:  report/standard_graphs/ple.27.7fg_YYYY.xml
...
### set up stock info
stk_info <- stockInfo(
  StockCode = "ple.27.7fg", 
  AssessmentYear = ass_yr, 
  ContactPerson = "vladimir.laptikhovsky@cefas.gov.uk", 
  Purpose = "Advice",
...
stk_info$StockSizeUnits <- "kg/km2" ### units: https://vocab.ices.dk/?ref=155
...
writeLines(xmlfile, paste0("report/standard_graphs/ple.27.7fg_", ass_yr, ".xml"))
...
if (isTRUE(verbose)) {
  ...
  key <- findAssessmentKey("ple.27.7fg", year = ass_yr)
  key_last <- findAssessmentKey("ple.27.7fg", year = ass_yr - 2) ### biennial
