...
catch_A <- catch %>%
  select(year, advice = advice_catch, discards = ICES_discards,
...
### ------------------------------------------------------------------------ ###
### 2 over 3 ratio
r <- rfb_r(idxB, units = "kg/100km")

### ------------------------------------------------------------------------ ###
### b - biomass safeguard ####
### ------------------------------------------------------------------------ ###
### do not redefine biomass trigger Itrigger and keep value defined in 2022
### Itrigger based on Iloss (1994)
b <- rfb_b(idxB, units = "kg/100km", yr_ref = 1994)
...
### Lc was defined at WGCSE 2022 by using data from 2018:2021
### keep this definition (do not update Lc)
#lc <- Lc(lngth, pool = 2018:2021, units = "mm")
lc <- Lc(235, units = "mm")
...
#lref <- Lref(basis = "LF=M", Lc = lc, Linf = 504.3004, units = "mm")
lref <- Lref(value = 302.3251, basis = "LF=M", units = "mm")
...
### UPDATE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
discard_rate <- catch %>%
  select(year, discards = ICES_discards, landings = ICES_landings,
         catch = ICES_catch) %>%
  filter(!is.na(catch)) %>%
